...
async function main() {
  ...
  try {
    ...
    // Load the mint_item.json file from ipfsMetasDir directory and parse it as JSON
    meta = JSON.parse(fs.readFileSync(`${FOLDERS.ipfsMetasDir}/${ACCOUNT_DETAILS.mint_item}.json`));
    ...
    try {
      ...
      if(mintedFile.length > 0) {
        ...
        if(mintedMeta.mintData.response !== "OK" || mintedMeta.mintData.error !== null) {
          ...
          const txnCheck = await fetchWithRetry(`${mintedMeta.mintData.transaction_external_url}`, options)
            .then(response => {
      ...
      try {
        ...
        const mintInfo = {
          ...
          mint_to_address: getMintAddress(meta.custom_fields.edition),
        ...
        const mintData = await fetchWithRetry("https://api.nftport.xyz/v0/mints/customizable", options);
    ...
    console.log(`File ${mintItem}.json does not exist, please double check mint_item and files available.`);
  }
}
